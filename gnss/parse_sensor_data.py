...
def save_feats(df, df_sat, feats, feat_lst, tag_2_feats, fn=FEAT_CKPT):
    data = {
        "df": df,
        'df_sat': df_sat,
        "feats": feats, 
...
def get_sat_records(df, idxs, df_sat, keep_atts=['fn', 'label', 'aTime', 'satCount', 'useSatCount']):
    ...
    db_50 = db_lst.apply(lambda x: np.quantile(x, .5))
